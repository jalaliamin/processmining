...
class OCEL:
    """
    Keeps .e2o as your internal representation.

    Only change vs your provided OCEL:
      - cast_to_pm4py now passes pm4py_ocel.relations := self.e2o (as pm4py expects).
      - cast_from_pm4py stores pm4py_ocel.relations back into self.e2o.
      - Everything else stays aligned with your original code and filenames.
    """
    ...
    def cast_to_pm4py(self):
        ...
        # pm4py OCEL signature: OCEL(events, objects, relations, ...)
        # Your internal e2o corresponds to pm4py_ocel.relations.
    ...
    def cast_from_pm4py(cls, pm4py_ocel):
        ...
        common_rows = pd.merge(
            melted_df,
            pm4py_ocel.object_changes,
            on=[lbl_object_id, lbl_object_type, lbl_object_attribute_changed_name],
            how="inner"
        )
        filtered_df1 = melted_df[
            ~melted_df.set_index([lbl_object_id, lbl_object_type, lbl_object_attribute_changed_name]).index.isin(
                common_rows.set_index([lbl_object_id, lbl_object_type, lbl_object_attribute_changed_name]).index
            )
        ]
        ...
        with pd.option_context("future.no_silent_downcasting", True):
            merged_df[lbl_timestamp] = merged_df[lbl_timestamp].infer_objects().fillna(default_timestamp)

        df_dts = [pm4py_ocel.events, pm4py_ocel.relations, merged_df]
        for df in df_dts:
            if df is not None and not df.empty and lbl_timestamp in df.columns:
                df[lbl_timestamp] = pd.to_datetime(df[lbl_timestamp], format="%Y-%m-%dT%H:%M:%S.%fZ", errors="coerce")

        # IMPORTANT: store pm4py_ocel.relations into e2o
    ...
    def save_to_jsonfile(self, file_path):
        ...
        result = {
            "objectTypes": build_object_types(self.objects) if self.objects is not None else [],
            "eventTypes": build_event_types(self.events) if self.events is not None else [],
            "objects": build_objects(self.objects, self.object_changes, self.o2o) if self.objects is not None else [],
            "events": build_events(self.events, self.e2o) if self.events is not None else []
        }
    ...
    def load_from_jsonfile(cls, file_path):
        ...
        for event in ocel_json.get("events", []):
            ...
            if relationships:
                event_details["ocel:typedOmap"] = [{lbl_object_id: rel["objectId"], lbl_qual: rel["qualifier"]} for rel in relationships]
        ...
        for obj in ocel_json["objects"]:
            ...
            if relationships:
                object_details["ocel:o2o"] = [{lbl_object_id: relation["objectId"], lbl_qual: relation["qualifier"]} for relation in relationships]
        ...
        for obj_id, obj in log["ocel:objects"].items():
            ...
            obj_data = {lbl_object_id: obj_id, lbl_object_type: obj_type, **obj["ocel:ovmap"]}

            if "ocel:o2o" in obj:
                for related_obj in obj["ocel:o2o"]:
                    target_id = related_obj[lbl_object_id]
                    qualifier = related_obj[lbl_qual]
                    o2o.append({lbl_object_id: obj_id, lbl_target_object_id: target_id, lbl_qual: qualifier})
        ...
        for event_id, event_data in log["ocel:events"].items():
            ...
            for _, relationship_details in event_relationships.items():
